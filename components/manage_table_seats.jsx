...
export default function ManageTableSeats() {
  ...
  const addTable = (max_seats, subject) => {
    ...
    let amt = updatedTables.tables.amount !== undefined ? updatedTables.tables.amount : 0
    updatedTables.tables[amt + 1] = newTable
    updatedTables.tables.amount = amt+1
    setTables(updatedTables);
  }
                    ...
                    <table className="w-full border-gray-500 border-4 text-center">
                        ...
                        return(
                            <tr key={index} onClick={() => handleRowClick(index)} className={selectedRow === index ? styles.selectedRow : guest.table !== 0 ? styles.normalRowDisabled : styles.normalRow}>
                                ...
                                {Object.keys(tables.tables).length > 0 && guest.table !== undefined && <td className={styles.td}>{guest.table == 0 ? 'None' : tables.tables[guest.table.toString()].subject}</td>}
                                {Object.keys(tables.tables).length == 0 && <td className={styles.td}>None</td>}
                            </tr>
                            )
                        })}
                        </tbody>
